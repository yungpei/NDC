...
# torch.cuda.set_per_process_memory_fraction(0.8, 0)  # 限制最多使用 80% 顯存
...
parser.add_argument("--grid_size", action="store", dest="grid_size", default=512, type=int, help="Size of output grid for testing [256, 512]")
...
def main():
    ...
    # 創建訓練/測試數據集
        ...
        for epoch in range(FLAGS.epoch):
            print(f"Epoch {epoch+1}/{FLAGS.epoch} 開始...")
            ...
            for i, data in enumerate(dataloader_train, 0):
                print(f"\nTraining batch {i+1}/{len(dataloader_train)}")

                # 訓練網路 (以 bool 和 float 輸出為例)
                if FLAGS.input_type in ["sdf", "voxel", "udf"]:

                    if net_bool:
                        print("Processing bool network with grid input...")
                        ...
                        print(f"Bool Loss: {loss_bool.item():.6f} | Accuracy: {acc_bool.item():.6f}")

                    if net_float:
                        print("Processing float network with grid input...")
                        ...
                        print(f"Float Loss: {loss_float.item():.6f}")

                elif FLAGS.input_type in ["pointcloud", "noisypc"]:

                    if net_bool:
                        print("Processing bool network with pointcloud input...")
                        ...
                        print("Processing float network with pointcloud input...")
            ...
            if epoch%FLAGS.checkpoint_save_frequency==FLAGS.checkpoint_save_frequency-1:
                ...
                for i, data in enumerate(dataloader_test, 0):
                    print(f"\n[Testing] Sample {i + 1}/{len(dataloader_test)}")
                    ...
                    if FLAGS.input_type in ["sdf", "voxel", "udf"]:
                        ...
                        with torch.no_grad():
                            if net_bool:
                                pred_output_bool = network_bool(gt_input)
                                print("Running boolean prediction...")
                            if net_float:
                                print("Running float prediction...")
                                pred_output_float = network_float(gt_input)
                        ...
                        with torch.no_grad():
                            if net_bool:
                                print("Running boolean prediction on point cloud...")
                                pred_output_bool = network_bool(pc_KNN_idx, pc_KNN_xyz, voxel_xyz_int, voxel_KNN_idx, voxel_KNN_xyz)
                            if net_float:
                                print("Running float prediction on point cloud...")
                                pred_output_float = network_float(pc_KNN_idx, pc_KNN_xyz, voxel_xyz_int, voxel_KNN_idx, voxel_KNN_xyz)
                    ...
                    print(f"Saved output mesh to: {FLAGS.sample_dir}/test_{i}.obj")

                    if i >= 32:
                        print("Test loop limited to 32 samples. Stopping.")
                        break
...
if __name__ == '__main__':
    torch.multiprocessing.freeze_support()  # 處理可執行文件情況
    main()  # 啟動主程式邏輯